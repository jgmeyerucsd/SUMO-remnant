...
compare.table=function(object1=mgpl.brp.all,
	table=mgpl.brp.all@data,
	natureobject=nature,
	ub=ub.psp,
	sumo=sumo.psp,
	methyl=methyl.psp,
	...
	output.table<-cbind(table,in.nature=naturecol,in.PSP.sumo=sumocol,in.PSP.ub=ubcol,in.PSP.acetyl=acetyl.col)


	table[,1]
	alllist<-paste(substr(table[,"protein"],start=4,stop=9),table[,1],sep="_")
	naturelist<-paste(substr(table[naturecol=="+","protein"],start=4,stop=9),table[naturecol=="+",1],sep="_")
	sumolist<-paste(substr(table[sumocol=="+","protein"],start=4,stop=9),table[sumocol=="+",1],sep="_")
	ublist<-paste(substr(table[ubcol=="+","protein"],start=4,stop=9),table[ubcol=="+",1],sep="_")
	acetyllist<-paste(substr(table[acetyl.col=="+","protein"],start=4,stop=9),table[acetyl.col=="+",1],sep="_")
	
	write.table(naturelist,file="natlist.tsv",quote=F,sep="\t",col.names=T,row.names=F)
	write.table(sumolist,file="sumo.tsv",quote=F,sep="\t",col.names=T,row.names=F)
	write.table(ublist,file="ub.tsv",quote=F,sep="\t",col.names=T,row.names=F)
	write.table(acetyllist,file="acetyl.tsv",quote=F,sep="\t",col.names=T,row.names=F)
	write.table(alllist,file="alllist.tsv",quote=F,sep="\t",col.names=T,row.names=F)
...
memberrows<-output.table[,60:64]
ncol(memberrows)
for(x in 1:5){
	memberrows[,x]<-memberrows[,x]=="+"
		
	}

venneuler(combinations=memberrows)
write.table(output.table,file="MG132plus.allquant.psp.nature.tsv",quote=F,sep="\t",col.names=T,row.names=F)

rep(times=nrow(memberrows),"TRUE")

cbind(IDs=rep(times=nrow(memberrows),"TRUE"),memberrows)->memrow
write.table(memrow,file="MG132plus.memrows.tsv",quote=F,sep="\t",col.names=T,row.names=F)
